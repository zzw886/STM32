...
u8 MPU6050_DMP_Init(void)
{
	...
	{	
		...
		mpu_set_sample_rate(DEFAULT_MPU_100HZ);	   	  		                        //设置采集样率,多久更新fifo，并产生int中断
		...
		dmp_set_fifo_rate(DEFAULT_MPU_100HZ);   	 			                            //设置采集样率,多久更新fifo，并产生int中断
		run_self_test();		                                                        //自检
...
//返回值:0,正常
//    其他,失败
u8 MPU6050_DMP_Get_Data(float *pitch,float *roll,float *yaw,short *gx,short *gy,short *gz,short *ax,short *ay,short *az)//（5MS  200HZ调用一下，给 DEFAULT_MPU_HZ 频率保持一致，或者int中断引脚读取）
